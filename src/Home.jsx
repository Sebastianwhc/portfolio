...
const Home = ({ lang, setLang }) => {
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden" style={{ position: 'relative', zIndex: 20, backgroundColor: 'transparent', color: '#e2e8f0', fontFamily: '"Inter", sans-serif' }}>
      ...
      {/* HERO SECTION */}
      <section id="about" className="min-h-screen flex flex-col items-center justify-center pt-20 pb-12 relative">
        ...
        <motion.div
          ...
          className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-cyan-400 opacity-60 hover:opacity-100 cursor-pointer hidden md:flex flex-col items-center gap-2"
          onClick={() => document.getElementById('about-details')?.scrollIntoView({ behavior: 'smooth' })}
        >
          <span className="text-xs font-mono uppercase tracking-widest text-[#94a3b8]">SCROLL</span>
          <ChevronDown size={24} />
        </motion.div>
      </section>

      {/* ABOUT (Global Engineering) */}
      <section id="about-details" className="py-20 relative">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12" style={{ position: 'relative', zIndex: 30 }}>
            {['all', 'data', 'robotics', 'hardware', 'academic'].map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold transition-all duration-300 ${filter === category ? 'scale-105' : ''
                  }`}
                style={{
