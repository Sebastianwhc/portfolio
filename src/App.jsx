...
import { 
  ...
  Microchip,
  Activity,
  ExternalLink, // Importamos icono para enlaces externos
  PlayCircle    // Icono para videos/simulaciones
} from 'lucide-react';
...
const content = {
  es: {
    ...
    skills: {
      ...
      categories: [
        { name: "Robótica & Aero", icon: <Bot size={18}/>, skills: ["ROS/ROS2", "Gazebo", "MATLAB", "Control Systems", "Flight Dynamics"] },
        { name: "Data Science & AI", icon: <Brain size={18}/>, skills: ["Python", "TensorFlow", "Pandas", "Computer Vision", "Deep Learning"] },
        { name: "Cloud & Backend", icon: <Server size={18}/>, skills: ["Google Cloud", "BigQuery", "SQL", "Docker", "Git"] },
        { name: "Hardware", icon: <Cpu size={18}/>, skills: ["C++", "Arduino", "Raspberry Pi", "PLC", "Sensors"] }
      ]
    },
    projects: {
  ...
  en: {
    ...
    skills: {
      title: "Tech Stack",
      categories: [
        { name: "Robotics & Aero", icon: <Bot size={18}/>, skills: ["ROS/ROS2", "Gazebo", "MATLAB", "Control Systems", "Flight Dynamics"] },
        { name: "Data Science & AI", icon: <Brain size={18}/>, skills: ["Python", "TensorFlow", "Pandas", "Computer Vision", "Deep Learning"] },
        { name: "Cloud & Backend", icon: <Server size={18}/>, skills: ["Google Cloud", "BigQuery", "SQL", "Docker", "Git"] },
        { name: "Hardware", icon: <Cpu size={18}/>, skills: ["C++", "Arduino", "Raspberry Pi", "PLC", "Sensors"] }
      ]
  ...
  fr: {
    ...
    skills: {
      ...
      categories: [
        { name: "Robotique & Aéro", icon: <Bot size={18}/>, skills: ["ROS/ROS2", "Gazebo", "MATLAB", "Systèmes de Contrôle", "Dynamique de Vol"] },
        { name: "Data Science & IA", icon: <Brain size={18}/>, skills: ["Python", "TensorFlow", "Pandas", "Computer Vision", "Deep Learning"] },
        { name: "Cloud & Backend", icon: <Server size={18}/>, skills: ["Google Cloud", "BigQuery", "SQL", "Docker", "Git"] },
        { name: "Matériel", icon: <Cpu size={18}/>, skills: ["C++", "Arduino", "Raspberry Pi", "PLC", "Capteurs"] }
      ]
    },
    projects: {
      title: "Projets",
...
const StarBackground = () => {
  ...
  return (
    // FIX: Usamos Z-Index 0 para el fondo (Capa Base)
...
// Narrativa de Scroll: Cohete, Dron y Satélite
const ScrollNarrative = () => {
  ...
  return (
    // FIX: Z-Index 10 para la narrativa (Capa Media)
...
const App = () => {
  ...
  return (
    // FIX: Z-Index 20 para el contenido (Capa Superior) y fondo transparente
    <div className="min-h-screen font-sans overflow-x-hidden" style={{ position: 'relative', zIndex: 20, backgroundColor: 'transparent', color: '#e2e8f0', fontFamily: '"Inter", sans-serif' }}>
             ...
             <div className="relative w-96 h-96">
                ...
                {[1, 2, 3].map((i) => (
                  <motion.div
                    ...
                    style={{ margin: i * 20, border: '1px solid rgba(6, 182, 212, 0.3)', boxShadow: '0 0 10px rgba(8, 145, 178, 0.1)' }}
                    animate={{ 
                      rotateX: [0, 360], 
                      rotateY: [0, 360],
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-32 h-32 rounded-full flex items-center justify-center relative z-10" style={{ background: 'linear-gradient(to bottom right, #2563eb, #06b6d4)', boxShadow: '0 0 50px rgba(6, 182, 212, 0.6)' }}>
                ...
                {/* Floating Icons */}
                <motion.div 
                  animate={{ x: [60, -60, 60], y: [-80, 80, -80], scale: [1, 0.8, 1] }}
                  transition={{ duration: 8, repeat: Infinity }}
                  className="absolute top-1/2 left-1/2 p-3 rounded-xl z-20 backdrop-blur-md"
                  style={{ backgroundColor: 'rgba(15, 23, 42, 0.8)', border: '1px solid rgba(168, 85, 247, 0.5)' }}
                >
                  <Plane color="#c084fc" size={24} />
                </motion.div>
                <motion.div 
                  animate={{ x: [-70, 70, -70], y: [50, -50, 50], scale: [0.8, 1, 0.8] }}
                  transition={{ duration: 10, repeat: Infinity }}
                  className="absolute top-1/2 left-1/2 p-3 rounded-xl z-20 backdrop-blur-md"
                  style={{ backgroundColor: 'rgba(15, 23, 42, 0.8)', border: '1px solid rgba(6, 182, 212, 0.5)' }}
                >
                   <Cog color="#22d3ee" size={24} />
                </motion.div>
             </div>
          </motion.div>
        </div>
            ...
            <div className="grid md:grid-cols-12 gap-12">
              <div className="md:col-span-4 flex flex-col gap-6">
                 <div className="p-6 rounded-2xl relative overflow-hidden" style={{ backgroundColor: 'rgba(10, 10, 21, 0.6)', border: '1px solid rgba(51, 65, 85, 0.5)', backdropFilter: 'blur(4px)' }}>
                    ...
                    {/* CERTIFICATIONS - Estilos en línea para márgenes y paddings */}
      ...
      <section id="experience" className="py-24 relative">
        <div className="container mx-auto px-6">
          ...
          {/* Estilos en línea para el espaciado vertical entre trabajos */}
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12" style={{ position: 'relative', zIndex: 30 }}>
            {['all', 'data', 'robotics', 'hardware', 'academic'].map((category) => (
              <button
                ...
                style={{
                  backgroundColor: filter === category ? '#0891b2' : 'rgba(30, 41, 59, 0.8)', // Más opaco para mejor contraste
                  color: filter === category ? 'white' : '#cbd5e1', // Texto más claro
                  border: filter === category ? '1px solid #06b6d4' : '1px solid #475569', // Borde más claro
            ...
            <AnimatePresence>
              {filteredProjects.map((proj, i) => (
                 ...
                 >
                    ...
                    {/* Project Icon/Visual Area */}
                    <div className="md:w-1/3 flex flex-col items-center justify-center p-8 relative overflow-hidden min-h-[200px]" style={{ backgroundColor: '#020617', borderRight: '1px solid #1e293b' }}>
                       {/* Placeholder Visual - COMMENT THIS OUT IF YOU USE IMAGES */}
                       {proj.category === 'robotics' ? <Bot size={64} color="#22d3ee" className="relative z-10"/> : 
                        proj.category === 'data' ? <LineChart size={64} color="#60a5fa" className="relative z-10"/> : 
                        proj.category === 'hardware' ? <Cpu size={64} color="#c084fc" className="relative z-10"/> :
                        <Database size={64} color="#34d399" className="relative z-10"/>}
                       <span className="text-[10px] mt-4 uppercase tracking-[0.2em] relative z-10 font-bold" style={{ color: '#64748b' }}>{proj.category}</span>

                       {/* UNCOMMENT THIS TO USE REAL IMAGES
                       <img 
                         src={proj.image} 
                         alt={proj.title} 
